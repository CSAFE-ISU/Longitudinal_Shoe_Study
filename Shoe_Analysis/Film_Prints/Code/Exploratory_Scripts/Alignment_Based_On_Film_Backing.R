...
shoe_misalign_plots <- function(dfrow) {
  ...
  png(filename = file.path(img_output_dir,
                           sprintf("FFT_Aligned_Equalized_%s%s_%s.png",
...
align_scan_data_fix <- select(scan_info, ShoeID, Shoe_foot, date, rep, Brand, Size, img) %>%
  mutate(img = purrr::map(img, function(x) {
    y <- img_pad_to_size(x, ceiling(dim(x)/100)*100, value = 1)
    EBImage::clahe(y, nx = 50, ny = 50)
  })) %>%
  mutate(rep = paste0("img", str_sub(rep, 1, 1))) %>%
  tidyr::spread(key = rep, value = img) %>%
  mutate(aligned = purrr::map2(img1, img2, fft_align))
...
shoe_misalign_plots <- function(dfrow1, dfrow2) {
  res <- dfrow1$aligned[[1]]$res
  res2 <- dfrow2$aligned[[1]]$res
  png(filename = file.path(img_output_dir,
                           sprintf("FFT_Aligned_CLAHEqualized_%s%s_%s.png",
                                   dfrow$ShoeID, dfrow$Shoe_foot, dfrow$date)),
      width = 600*4, height = 2*600*2, units = "px", res = 300)
  layout(matrix(c(1, 2, 3, 6, 1, 4, 5, 6), nrow = 2, byrow = T))
  ShoeScrubR:::plot_imlist(res)
  ...
  title(main = "Original Print")
  plot(res2[[1]])
  title(main = "CLAHEqualized")
  plot(res[[2]])
  plot(res2[[2]])
  ShoeScrubR:::plot_imlist(res2)
  title(main = "CLAHEqualized\nFFT Aligned Shoe")
  dev.off()
}

purrr::map(misaligned, ~shoe_misalign_plots(align_scan_data[.,], align_scan_data_fix[.,]))
