...
In order to image the shoes using the EverOS scanner, lab assistants wore the participants' shoes while traversing the scanner in two different ways: normal walking, and a more stationary ``detail" print. Examples of these images are shown in \autoref{fig:everos-imgs}. Two replicates of each shoe and image type combination were taken, for a total of 8 2D scans per measurement point.
...
\begin{figure}
\includegraphics[width=.49\textwidth]{figure/3d_shoes_Participant_005,_Handheld_scan.png}
\includegraphics[width=.49\textwidth]{figure/3d_shoes_Participant_005,_Turntable_scan.png}
\includegraphics[width=.49\textwidth]{figure/3d_shoes_Participant_007,_Handheld_scan.png}
\includegraphics[width=.49\textwidth]{figure/3d_shoes_Participant_007,_Turntable_scan.png}
...
user_mat <- list(
  structure(c(0.863850295543671, 0.105972096323967, 0.492470979690552,
              0, 0.0217040628194809, -0.984541714191437, 0.173786520957947,
              0, 0.503276109695435, -0.1394372433424, -0.852798640727997, 0,
              0, 0, 0, 1), .Dim = c(4L, 4L)),
  structure(c(-0.866365671157837, -0.178002715110779, 0.46656459569931,
              0, 0.00676984805613756, 0.930016338825226, 0.367393344640732,
              0, -0.499316841363907, 0.321463465690613, -0.804542541503906,
              0, 0, 0, 0, 1), .Dim = c(4L, 4L)),
  structure(c(0.962948977947235, 0.108996115624905, 0.246657997369766,
              0, 0.0736871063709259, -0.986215353012085, 0.148127943277359,
              0, 0.259404003620148, -0.124464869499207, -0.957711100578308,
              0, 0, 0, 0, 1), .Dim = c(4L, 4L)),
  structure(c(0.995098650455475, 0.086215928196907, 0.0482813268899918,
              0, 0.058195274323225, -0.90621030330658, 0.418792247772217, 0,
              0.0798620134592056, -0.41393256187439, -0.906792163848877, 0,
              0, 0, 0, 1), .Dim = c(4L, 4L))
)
...
purrr::pwalk(list(scan_3d$stl_center, user_mat, 1:4, scan_3d$name), function(x, y, z, z1) {
  ...
  rgl::title3d(z1)
  rgl::snapshot3d(here::here(sprintf("Paper/figure/3d_shoes_%s.png", gsub(" ", "_", z1))))
  if (z != 4) rgl::clear3d()
})
@
...
In order to record the shoe surface in great detail, prints were collected using clear adhesive film and fingerprint powder. The shoe's surface was dusted with fingerprint powder, with excess powder removed using compressed air. A research assistant carefully put the shoe on, placing the heel of the shoe against the edge of the adhesive film, and used a roll step to ensure that the entire shoe sole made contact with the adhesive film. The film backing was then reapplied to the adhesive film using a roller; the database contains high-resolution scans of these completed prints. A second print was taken by pressing the adhesive film against the shoe without the full roll step to mimic the normal contact surface of the shoe. For 25 of the 160 pairs of shoes in the study, an additional replicate of each print was recorded in order to assess variability between repeated measurements of the same shoe. Examples of the walking and
...
\caption{Scans of the left shoes of participants 005 and 007, at the first and last data collection points. \label{fig:film-powder}}
\end{figure}
