...
lss_location <- if (Sys.info()[["sysname"]] == "Linux") {
  "/lss/research/csafe-shoeprints/ShoeImagingPermanent/"
} else {
...
<<everos-imgs, fig.cap = "Digital images taken using the EverOS v2.0 scanner. Images are from the left shoes of participants 005 and 007, taken during the initial visit (before participants had worn the shoes).", cache.lazy = FALSE, fig.width = 8, fig.height = 8/5*2>>=
...
At each measurement point, shoes were also scanned with a three-dimensional scanner; specifically, the EinScan-Pro+ scanner\footnote{\url{https://www.einscan.com/handheld-3d-scanner/einscan-pro-plus/}}. The sole of each shoe was scanned using ``Handheld Rapid Scan" mode from approximately 12" above the shoe sole to produce a STL file characterizing the surface.

After the final data collection point, shoe soles were scanned using the scanner's auxiliary turntable, which produces much higher resolution scans.
...
user_mat <- list(
  structure(c(0.937049567699432, -0.0810819789767265, 0.339646428823471,
              0, -0.0187115967273712, -0.982927978038788, -0.18302583694458,
              0, 0.348688900470734, 0.165149331092834, -0.922571241855621,
  ...
  structure(c(0.962948977947235, 0.108996115624905, 0.246657997369766,
              ...
              0, 0, 0, 0, 1), .Dim = c(4L, 4L)),
  structure(c(0.970346808433533, 0.0750713720917702, 0.229732573032379,
              0, -0.034701082855463, -0.897408306598663, 0.439823895692825,
              0, 0.239185556769371, -0.434756517410278, -0.868197739124298,
              0, 0, 0, 0, 1), .Dim = c(4L, 4L))
)

rgl::open3d(windowRect = c(0, 0, 1000, (2 + .25)*round(1000/2.5)), zoom = .75)
rgl::layout3d(matrix(1:4, nrow = 2), sharedMouse = F)

purrr::pwalk(list(scan_3d$stl_center, user_mat, 1:4, scan_3d$name), function(x, y, z, z1) {
  rgl::triangles3d(x, aspect = "iso", col = "red", zoom = .75)
  rgl::view3d(userMatrix = y)
  Sys.sleep(1)
  # rgl::axes3d(expand = 1.1)
  rgl::title3d(main = z1)
  rgl::rgl.bringtotop()

  if (z != 4) rgl::next3d(clear = F)
})
