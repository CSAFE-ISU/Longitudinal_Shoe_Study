...
data_location <- here::here("Paper/sampledata")
@
...
\begin{abstract}
In this paper, we describe the data collected as a result of a longitudinal study of athletic shoe wear over the course of approximately 6 months. During the 6-month period, we collected impressions and digital scans of the shoes at 4 intervals. The data generated from this study will serve as a publicly accessible resource for future studies of RAC distribution, wear pattern development, automated identification algorithm assessment, and examiner assessments. This paper describes the collection methods and available data with the goal of introducing the forensic science community to this unparalleled trove of research data.
\end{abstract}

\begin{keyword}
forensic science, footwear, athletic footwear, database, wear, randomly acquired characteristics, longitudinal study
\end{keyword}
...
We selected two common models of athletic shoes: Adidas Seeley and Nike Winflo 4; by controlling the shoe models available, we can provide several sets of data for each size/model combination. In addition, the two shoes have different outsole materials: Adidas Seeley shoes have a hard rubber sole, while Nike Winflo shoes have a much softer rubber and foam construction. We expected that these shoes would accumulate RACs at different rates based on material differences. The Nike shoes also have a greater variety of different geometric elements at different depths: this should produce variation in the types of elements which are visible at different levels of shoe wear. The Adidas shoes, on the other hand, have no variability in the elements visible at different tread depths; the geometric pattern on the sole should remain the same despite wear.

<<sizes, echo = F, include = T, render='asis'>>=
shoe_model_data <- read.xlsx(file.path(data_location, "../../shoeprints data.xlsx")) %>%
  select(1, 2, 4:5, 11) %>%
...
Pressure scans were recorded during the initial participant visit (that is, when participants were first issued shoes), using a Tekscan HR Mat scanner with a sensor resolution of $0.5 \text{cm}^2$ and temporal resolution of 185 Hz\footnote{\url{https://www.tekscan.com/products-solutions/systems/hr-mat}. Cached version available at \url{https://webcache.googleusercontent.com/search?q=cache:BZBl2bvx-H4J:https://www.tekscan.com/products-solutions/systems/hr-mat+}}. Participants walked across the mat 12 times, resulting in 3 scans of each foot while wearing a shoe, and 3 scans of each foot while wearing only a thin disposable sock. Videos and still images of the pressure distribution were saved; frames from the video and still images are shown in Figures \ref{fig:pressurescanner-video} and \ref{fig:pressurescanner-static}, respectively. The pressure scan data are archived separately \citep{mat-scanner} and are also available as part of the larger collection \citep{collection}.
...
<<pressurescanner-video, fig.cap = "4 equally spaced frames from the video of participants 005 and 007 walking across the pressure scanner, with and without shoes.", echo = F, fig.width = 8, fig.height = 4, cache = F>>=
pressure_videos <- tibble(
  file = list.files(data_location, pattern = "^00[57]\\d{3}L_\\d{8}_1_[16]_1_.*\\.csv", full.names = T),
...
pressure_stills <- tibble(
  file = list.files(data_location, pattern = "^00[57]\\d{3}L_\\d{8}_1_[27]_1_.*\\.csv", full.names = T),
  frame_data = purrr::map(file, read_csv_video)
) %>%
  mutate(
    participant = str_extract(file, "005|007"),
...
In order to image the shoes using the EverOS scanner, lab assistants wore the participants' shoes while traversing the scanner in two different ways: normal walking, and a more stationary ``detail" print. Examples of these images are shown in \Cref{fig:everos-imgs}. Two replicates of each shoe and image type combination were taken in order to facilitate estimation of variability in print features \citep{shorInherentVariationMultiple2018}, for a total of 8 2D scans per measurement point. The 2D scans are available separately \citep{2dscan} and as part of the data collection \citep{collection}.
...
everos_imgs <- list.files(data_location, pattern = "^00[57]\\d{3}L_2017\\d{4}_2_[12]_1_.*", full.names = T)
...
At the conclusion of the study, the shoe soles were scanned using the scanner's auxiliary turntable, which produces much higher resolution scans. Low and high resolution scans of the left shoes of participants 005 and 007 are shown in \Cref{fig:3dscans}. The 2D scans are available separately \citep{3dscan} and as part of the data collection \citep{collection}.
...
<<3d-scans, dev="png", eval = FALSE>>=
scan_3d <-
  tibble(
    file = list.files(data_location, pattern = "^00[57]\\d{3}L_\\d{8}_3_[12]_1_.*", full.names = T),
    participant = str_extract(file, "005|007"),
...
A Canon EOS 5D Mark IV camera was used to take photographs of the shoe soles\citep{photos}. Shoes were positioned light-colored surface, with a ruler in the camera frame to provide scale information. Two images were captured for each shoe.

During the experiment, the photography procedure changed slightly on recommendations from practitioners and statistical analysts. L-rulers were added on a stand to be level with the shoe sole, and the background of the image was changed to make the images easier to work with. A summary of the changes to the photography procedure can be found in the appendix, but most changes are easily discernible from the example images in \Cref{fig:dslr-photos}.
...
\section{Data Access}
All of the data described in the previous section is publicly available at the \href{https://forensicstats.org/shoeoutsoleimpressionstudy/}{CSAFE Forensic Science Data Portal}\footnote{\url{https://forensicstats.org/shoeoutsoleimpressionstudy/}}, along with relevant metadata (e.g. participant weight, number of steps at each data collection point, surfaces walked across) and detailed data collection procedures describing exactly how the data was collected and any changes made in the data collection procedures during the experiment (such as the addition of the square rulers in photographs).
The Data Portal site links to a visual database which can be used to select subsets of the data described in this paper for specific analysis projects.
Additional bulk downloads of the collection\citep{collection} are available at \url{http://doi.org/10.25380/iastate.c.4477757}; the collection also links to individual bulk downloads of data from each collection method.

\section{Discussion}
The data described in this paper are well beyond the scope of any currently available, free, public research database of shoe prints. We envision many different potential analyses which could be conducted using these data, including examinations of the power of each method to capture RACs, wear, and other features. The data sets may also allow for comparisons of wear pattern development over time (with corresponding analysis of pressure individual pressure pattern distributions), studies of the effects of different kinematics using the paper prints, and development of alignment and automatic identification methods for athletic shoes. In addition, we hope that these data will be one resource for conducting white and black-box examiner studies, as there are multiple different shoes of the same size and type in this data set, providing the opportunity to examine comparisons within shoe models. This study is not intended to provide material which exactly emulates case work: we expect it will be far more useful to the research community than to examiners directly, because the study's design explores so many different factors of interest in footwear research.

While the data described here are extensive and well beyond the scope of any currently available public database of shoe prints, we did encounter some challenges and limitations during the study which may impact the results. The data collected consist of snapshots of the best procedures we had for data collection at the time; the deficiencies in these procedures which became obvious during the course of the study can only be addressed in future studies. In several cases, we discovered during the course of the experiment better or more reliable methods of capturing the data, leading to changes in collection procedures. The collection procedures and any changes to those procedures are available \citep{procedures}; notable changes include modifications of the Digital Camera procedure to include an L-shaped ruler at the precise height of the shoe sole as well as a background which could be more easily identified and isolated from the shoe sole. At the end of the study, we discovered that a turntable was available for the 3D scanner, facilitating the capture of data at higher resolution; as a result, the final 3D scans are of considerably higher resolution than the initial scans. The film used to collect the film and powder prints is of two different varieties (provided by the same supplier) with different patterns of the backing paper and a slightly different tint. In many cases, these changes only peripherally affect the data which were recorded.

Due to logistical limitations, the data captured while the shoe was being worn (2D scanner, film and powder, vinyl and powder, paper and powder) could not be captured using the participant and primary wearer of the shoe; instead, in these cases, research assistants wore the shoes while data were recorded. While this made subject recruitment considerably easier and decreased the time commitment required of participants, this decision did create some additional artifacts in the data: due to variation in weight and height of the research assistants, the prints were not recorded under the exact same conditions at each time point. If we were to repeat this experiment subject to the same limitations on participant time, we would at least control for the variability by ensuring that the same research assistants wore the same shoes during data collection at each time point.

There are obviously additional limitations to these data introduced by the inclusion of only two makes and models of athletic shoes. Both shoes have highly repetitive pattern elements which make development of automatic analysis algorithms extremely challenging. The repetitive pattern elements consistently result in mis-alignments when automatic alignment algorithms are applied to multiple images of the same shoe, even at the same time point. While we have begun to analyze the data ourselves and plan to release an R software package designed to facilitate the preprocessing of the data, that work is ongoing and will be reserved for a future publication. It is more important, in our view, to make the data available to the community quickly for exploration and analysis by other researchers. The use of two makes and models of shoes also makes research into similarity score testing using this database both deep (in the number of potential challenging comparisons) and somewhat limited in scope\citep{soyoung}. While there are many comparisons to be made between shoes worn by different individuals, any conclusions made based on data from this study alone may not generalize  well to the wider population of athletic shoes or more generally, all shoes.

\section{Applications and Future Work}
We are planning several follow-up studies which will expand the database and leverage the methodological testing from this experiment to produce more precise results. We hope to collect data for a supplemental database containing  shoes with different brands, patterns, and manufacturing methods/material composition. In addition, we are in the process of testing procedures for recording data similar to that which might be found at a crime scene, including simulated prints in dust as well as impressions in dirt and mud and casts of those impressions. We plan to use the shoes from this study to create prints which can then be used for more realistic examiner testing.
